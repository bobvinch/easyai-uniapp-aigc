...
export type IDrawHistoryItemCreat = MakeRequired<
  Omit<IDrawHistoryItem, 'user_id'> & { user_id: string },
  'user_id' | 'workflow_id'
>
...
export interface IDrawTaskStatus {
  task_id: string
  status: 'started' | 'progressing' | 'success' | 'failed' | 'waiting'
  progress?: number
  server: string
  queue: number
...
/** 微信小程序创建支付订单 */
export type IPayOrderCreatWechatMiniProgramPay = Omit<
  MakeRequired<IPayOrder, 'amount' | 'payer' | 'user_id' | 'product_id' | 'order_channel'>,
  'product_id'
> & { product_id: string }
